...
- **Recover explicitly.** A supported browser-created child tab can continue from its exact
  opener. Stale workspaces and closed MCP transport have concrete recovery actions instead of
  silent tab switching or blind retries.
...
[compatibility map](compatibility.json) is authoritative; `doctor` does not guess the live adapter
version. The [public status file](docs/public-status.json) owns current release, platform, and
store state.

Ghostlight 0.8 supports exact local stdio MCP revisions `2025-11-25` and `2026-07-28`. That is
source-candidate compatibility, not a claim that the 0.8 service is already public. Use the
[changelog](CHANGELOG.md) and public status above for release state.
...
The connector owns the MCP revision state machines. The persistent service owns workspaces, the
tool catalog, scheduling, governance, browser coordination, and audit. The browser connector is a
native-messaging pass-through, and the extension owns Chrome mechanism without policy. See
[ADR-0096](docs/adr/0096-protocol-versioned-mcp-edge-and-neutral-service.md) for the executable
boundary and [docs/SPEC.md](docs/SPEC.md) for the deeper governance model.
