...
-- restart the client to load the server, and add the browser extension if the test shows it
...
commented JSONC config cannot be merged safely, preserve the file and use the exact entry the
installer prints; configuration guidance is not an installation failure.

For a client the installer does not recognize, add this entry after running the command:
...
by step 1 and install [Ghostlight in Browser from the Chrome Web Store](https://chromewebstore.google.com/detail/ghostlight-in-browser/lejccfmoeogmhemakeknjjdhkfkgncdl).
...
- Errors or not connected -> the browser extension is the likely cause (the one piece the CLI can't
  ...
  3. If the extension is disabled, have the user enable it from the browser's extension manager,
     then retest.
