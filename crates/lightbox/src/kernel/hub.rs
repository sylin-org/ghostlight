...
pub(super) fn registry() -> Vec<Scenario> {
    vec![
        ...
        ("kernel-hub-kill-audit-fanout", kill_audit_fanout),
    ]
}
...
fn two_mcp_edge_multiplex() -> anyhow::Result<()> {
    ...
    let tab_a = support::creator_tab_handle(&context_a)?;
    ...
    let tab_b = support::creator_tab_handle(&context_b)?;
    ensure!(tab_b != tab_a);

    write_line(
        &mut stdin_a,
        &json!({"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"browser_navigate","arguments":{"tab":tab_a,"url":"https://a.example.com"}}}),
    )?;
    write_line(
        &mut stdin_b,
        &json!({"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"browser_navigate","arguments":{"tab":tab_b,"url":"https://b.example.com"}}}),
    )?;
    let reply_a = read_line(&mut reader_a)?;
    let reply_b = read_line(&mut reader_b)?;
    ensure!(
        reply_a["id"] == 3
            && reply_a["result"]["structuredContent"]["tab"]["id"] == tab_a
            && !reply_a.to_string().contains("tabId=101")
    );
    ensure!(
        reply_b["id"] == 3
            && reply_b["result"]["structuredContent"]["tab"]["id"] == tab_b
            && !reply_b.to_string().contains("tabId=202")
    );
