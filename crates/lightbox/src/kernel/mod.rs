...
//! Named parity scenarios for the 27 kernel spawn tests (ADR-0056 Decision 3). Each scenario moves
...
/// The migrated kernel scenario registry.
...
fn control_status() -> anyhow::Result<()> {
    let tmp = TempRoot::new("kernel-control-status")?;
