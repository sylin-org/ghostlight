...
#[cfg(test)]
impl TestResponse {
    fn success(_id: Option<Value>, result: Value) -> Self {
        Self {
            result: Some(result),
            error: None,
        }
    ...
    fn error(_id: Option<Value>, code: i64, message: impl Into<String>) -> Self {
        Self {
            result: None,
...
/// Test adapter around the protocol-neutral dispatch chokepoint.
#[cfg(test)]
...
/// Test entry using a reloadable atomic authority store.
#[cfg(test)]
async fn handle_tools_call_scheduled(
    browser: &Browser,
    store: &Arc<ConfigStore>,
    ...
    let Some(name) = params.and_then(|p| p.get("name")).and_then(Value::as_str) else {
        return TestResponse::error(id, -32602, "tool call requires a string 'name'");
    };
    let args = params
...
#[cfg(test)]
fn render_outcome(id: Option<Value>, outcome: CallOutcome) -> TestResponse {
    match outcome {
        CallOutcome::Success { result } => TestResponse::success(id, result),
        CallOutcome::Failure { error } => TestResponse::success(id, error_result(error)),
        CallOutcome::NotDispatched { message } => {
            TestResponse::success(id, execution_status_result("not_dispatched", true, message))
        }
        CallOutcome::OutcomeUnknown { message } => TestResponse::success(
            id,
...
/// Execute one protocol-neutral service work item through the canonical tool pipeline.
...
mod tests {
    ...
    use crate::tool::outcome::LocalFuture;
    ...
    /// ADR-0060/0096: a tighten-only request restriction denies a target the SERVICE allows, and permits
    ...
    /// Immutable client presentation on one neutral work item reaches its audit record.
    #[tokio::test]
    async fn neutral_work_audit_uses_its_own_client_identity() {
        let path = temp_audit_path("client-context");
        let _ = std::fs::remove_file(&path);
        let recorder = Arc::new(Recorder::to_file(path.clone()));
        let governance = Arc::new(Governance::all_open(recorder as Arc<dyn AuditSink>));
        let store =
        ...
        .await;
        assert!(matches!(outcome, CallOutcome::Failure { .. }));

        let lines = read_lines(&path);
        assert_eq!(lines.len(), 1);
        assert_eq!(lines[0]["client"]["name"], "test-client");
        assert_eq!(lines[0]["client"]["version"], "9.9.9");
        std::fs::remove_file(&path).ok();
    }
    ...
    #[tokio::test]
    async fn stale_reentrant_lease_fails_instead_of_reacquiring_forever() {
        let store =
            crate::governance::config::reload::ConfigStore::for_test_with_config(Config::minimal());
        let recorder = Arc::new(Recorder::disabled());
        let governance = Arc::new(Governance::all_open(recorder as Arc<dyn AuditSink>));
        let authority =
            AuthorityStore::from_existing(&store.current_authority(), Arc::clone(&governance));
        let admitted = authority.current();
        let scheduler = crate::hub::scheduling::CommandScheduler::default();
        scheduler.advance_authority_epoch(admitted.epoch);
        let execution = scheduler
            .acquire(
                crate::hub::scheduling::ScheduleKey::Surface(
                    crate::hub::scheduling::BrowserSurface {
                        browser_slot: 1,
                        native_tab: 7,
                    },
                ),
                crate::hub::scheduling::ProducerId::new("test"),
                admitted.epoch,
            )
            .await
            .unwrap();
        let newer = Arc::new(AuthoritySnapshot {
            config: admitted.config.clone(),
            governance: admitted.governance.clone(),
            policy: admitted.policy.clone(),
            epoch: admitted.epoch + 1,
        });

        assert!(matches!(
            reentrant_authority_snapshot(&execution, None, newer.clone()),
            Err(ScheduleFailure::AuthorityChanged)
        ));
        let selected = reentrant_authority_snapshot(&execution, Some(&admitted), newer).unwrap();
        assert!(Arc::ptr_eq(&selected, &admitted));
    }
}
