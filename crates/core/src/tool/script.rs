...
pub(crate) struct PipelineRunner<'a> {
    ...
    /// ADR-0060/0096: this call's tighten-only restriction, threaded from the `LocalCtx` so every
    /// orchestrated sub-step is bound by the SAME tighten-only restriction as its parent call.
    ...
    pub(crate) work: Option<&'a WorkContext>,
    pub(crate) cancellation: Option<&'a CancellationToken>,
    pub(crate) workspaces: Option<&'a WorkspaceRegistry>,
}
...
fn step_text(outcome: &CallOutcome) -> Option<String> {
    let result = match outcome {
        ...
        CallOutcome::Cancelled { message } => return Some(message.clone()),
    };
    result
        .get("content")
        .and_then(Value::as_array)
...
pub(crate) fn run_batch<R: StepRunner>(
    ...
    for (i, step) in steps.iter().enumerate() {
        ...
        if runner.is_cancelled() {
            for j in i..steps.len() {
                let tool = steps
                    .get(j)
                    .and_then(|candidate| candidate.get("tool"))
                    .and_then(Value::as_str)
                    .unwrap_or("?")
                    .to_string();
        ...
        if matches!(status, "held" | "attention_required" | "cancelled")
    ...
    if let Some(stop) = stopped_at {
        if (stop as usize) < steps.len()
            && !matches!(
                stop_reason,
                StopReason::Budget | StopReason::None | StopReason::Cancelled
            )
        {
...
fn summarize(reason: StopReason, stopped_at: Option<u32>, completed: u32, total: u32) -> String {
    match reason {
        ...
        StopReason::Cancelled => {
            let k = stopped_at.unwrap_or(completed + 1);
            format!("{completed}/{total} steps completed; cancelled before step {k}")
        }
    }
}
...
// ADR-0047 D3 threads the browser-wire `guid` workspace routing key through to run_tool_call,
// pushing this bridge to 8 params; the arity mirrors run_tool_call's pinned signature, so the lint
// is allowed here too.
...
fn acquire_retained_surface(
    ...
    tokio::task::block_in_place(|| {
        ...
        acquired.map_err(schedule_failure_message)
    })
}
...
mod tests {
    ...
    fn clone_outcome(o: &CallOutcome) -> CallOutcome {
        match o {
            ...
            CallOutcome::Denied { message, source } => CallOutcome::Denied {
                message: message.clone(),
                source: match source {
                    crate::tool::outcome::DenialSource::Policy => {
                        crate::tool::outcome::DenialSource::Policy
                    }
                    crate::tool::outcome::DenialSource::Sacred => {
                        crate::tool::outcome::DenialSource::Sacred
                    }
                },
            },
            ...
            CallOutcome::Cancelled { message } => CallOutcome::Cancelled {
                message: message.clone(),
            },
        }
    }
    ...
    fn denied() -> CallOutcome {
        CallOutcome::Denied {
            ...
            source: crate::tool::outcome::DenialSource::Policy,
        }
    }
