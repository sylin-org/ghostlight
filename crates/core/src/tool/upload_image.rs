...
//! from the per-workspace screenshot cache on [`Browser`] by `imageId` -- to a file input (`ref`) or a
...
async fn run(
    ...
    match browser
        .call_with_delivery_outcome(guid, "upload_image_exec", &exec_args, execution)
        .await
    {
