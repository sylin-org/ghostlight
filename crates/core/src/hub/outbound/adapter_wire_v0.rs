...
use crate::browser::mechanism::{BrowserControl, BrowserControlId, MechanismId, MechanismRequest};
use crate::ToolError;
...
fn legacy_tool(request: &MechanismRequest) -> Result<(&'static str, Value), ToolError> {
    ...
    let tool = match request.id() {
        ...
        NavigateUrl => {
            args.remove("readiness");
            "navigate"
        }
        NavigateBack => {
            args.remove("readiness");
            field(&mut args, "url", "back", "navigate")
        }
        NavigateForward => {
            args.remove("readiness");
            field(&mut args, "url", "forward", "navigate")
        }
        NavigateReload => {
            args.remove("readiness");
            action(&mut args, "reload", "tab_control")
        }
        WorkspaceTabOpen | NavigationAwaitReadiness | NavigationVerifyDocument => {
            return Err(ToolError::binary(format!(
                "mechanism {} requires navigationReadinessV1",
                request.id()
            )))
        }
        PageSnapshot => {
            args.remove("canonical_targets");
            rename(&mut args, "scope_ref", "ref_id");
            "read_page"
        }
        PageReadText => {
            flatten_target_reference(&mut args)?;
            rename(&mut args, "ref", "ref_id");
            "get_page_text"
        }
        PageFind => "find",
...
mod tests {
    ...
    #[test]
    fn unflagged_form_set_value_keeps_its_byte_exact_legacy_wire() {
        let bytes = serialize_tool_request(
            "8",
            "workspace-1",
            &request(
                MechanismId::FormSetValue,
                json!({"tab":9,"target":{"ref":"field-1"},"value":"ordinary"}),
            ),
            &json!({"class":"scheduled"}),
            None,
        )
        ...
        assert_eq!(
            String::from_utf8(bytes).unwrap(),
            r#"{"id":"8","type":"tool_request","tool":"form_input","args":{"value":"ordinary","tabId":9,"ref":"field-1"},"guid":"workspace-1","resultFeatures":["tabDeltaV1"],"execution":{"class":"scheduled"}}"#
        );
    }

    #[test]
    fn canonical_readiness_is_not_added_to_the_covered_legacy_navigation_wire() {
        let bytes = serialize_tool_request(
            "7",
            "workspace-1",
            &request(
        ...
        assert_eq!(
            value["args"],
            json!({"url":"https://example.com","tabId":9})
        );
    }
    ...
    fn every_typed_mechanism_has_an_exhaustive_legacy_serialization_decision() {
        for id in MechanismId::ALL {
            ...
            if matches!(
                ...
                assert!(legacy_tool(&request).is_err(), "{id}");
            } else if *id == MechanismId::TabUrlQuery {
                ...
                let expected_tool = match id {
                    ...
                    | MechanismId::NavigationVerifyDocument => unreachable!(),
                };
