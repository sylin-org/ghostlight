...
treatment. ADR-0087 defines privacy-safe key presentation. ADR-0089 closes the destination-cue
and backstage-inspection review.
...
| `computer.scroll_to` by ref | Spatial | chevrons settle into the exact target halo | Semantic composition suppresses a duplicate halo |
| `computer.scroll_to` by coordinate | Native | visible page movement | Compatibility path has no honest element target |
...
| `read_console_messages` | Quiet | none | Reads an extension-held diagnostic buffer, not the rendered page |
| `read_network_requests` | Quiet | none | Reads an extension-held diagnostic buffer, not the rendered page |
...
| `upload_image` by coordinate | Spatial | fixed photo tile enters the target halo | Says dispatch, never acceptance; carries no image metadata |
...
| `gif_creator.export` | Native or spatial | browser download UI or image-drop target | Reuses the coordinate image-drop cue; no umbrella badge |
| `explain` | Quiet | none | Local MCP directory only |

## Review queue

The v1 tool surface has no unresolved visual-signature decisions. New tools and action variants
must be classified here before implementation.
