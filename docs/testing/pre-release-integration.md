...
| Browser capabilities | Every catalog tool plus semantic/coordinate input, nested frames and shadow roots, capture reuse/upload, dialogs, recording delivery/erase, history, explicit agent tab creation and group reuse, stale handles, and bounded diagnostics. | Expanded installed journey plus Foundry and the existing frame/script journeys. Mark unsupported variants separately. |
| Governed work | Configure real local/managed policy in a dedicated test account. Exercise independent RAWX, all-open, allow/deny for agent requests and returned landings, redirects, excluded documents, bad cold policy, last-valid reload, and configured credential handoff. | Isolated governance and real Chromium tests already exist; repeat key allowed and denied jobs through the registered native host. Never edit the owner's policy for this. |
| Human browsing | Manual navigation, including a protected URL, creates no Ghostlight activity, audit, policy response, notification, or tab hold. Child tabs and popups are not automatically adopted, grouped, or attached to a debugger, including during agent work. Manual tab movement causes no regrouping; manual closure remains available. Cached targets expire, and the next agent request checks the current destination. | [ADR-0164](../adr/0164-govern-only-ghostlight-work.md) has service and extension regressions; repeat visibly with the candidate service and loaded adapter, both idle and during agent work. An old adapter can still group or debug child tabs. |
| Human controls | Actual Pause/Stop/session recovery prevents subsequent agent effects and remains usable during load. Preserve-tabs blocks agent close. Workbench controls and notification match receipts. | Existing process/desktop/history tests supplement actual visible installed interaction. |
