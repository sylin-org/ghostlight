...
- STOP precondition: release.yml DID have a raw version-less binary upload, so the raw-triple change applied (no adapt-and-skip needed).
  ...
  4. Scoop `bin` became a three-element array; WinGet gained two more `NestedInstallerFiles` with
     `PortableCommandAlias` per adapter. `DISTRIBUTION.md`, `README.md`, and `CLAUDE.md` name the
     three executables. `tests/e2e/run-smoke.mjs` was already wired in S6.
